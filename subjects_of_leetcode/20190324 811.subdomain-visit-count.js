...
var subdomainVisits = function(cpdomains) {
    ...
    cpdomains.forEach(element => {
        countTimes(element, map);
    });
    for(let key of map.keys()) {
        visits.push(map.get(key).toString() + ' ' + key);
    }
    return visits;
    function countTimes(domain, map) {
        let number = parseInt(domain.split(' ')[0]);
        let subdomains = domain.split(' ')[1].split('.');
        let len = subdomains.length;
        for(let i = len - 1;i >= 0;i--) {
            let sub = subdomains.slice(i).join('.');
            if(map.get(sub)) {
                map.set(sub, map.get(sub) + number);
            } else {
                map.set(sub, number);
            }
        }
